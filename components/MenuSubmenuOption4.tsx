...
import { ChevronDown, X, Menu, Plus } from 'lucide-react';
...
export default function MenuSubmenuOption4() {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden lg:flex items-center justify-center h-16 space-x-8">
          ...
          <div className="relative group">
            ...
            {/* Submenu Option 4: Single Wide Column - Minimal and Spacious */}
            <div className="absolute left-1/2 -translate-x-1/2 mt-0 w-[420px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                ...
                <ul className="space-y-1">
                  {menuData.services.map((item) => (
                    <li key={item.title}>
                      <a
                        href={item.href}
                        className="block py-2.5 px-4 rounded-md transition-all hover:bg-gray-50"
                        style={{ color: 'rgb(55, 68, 109)', fontWeight: 400 }}
                        onMouseEnter={() => item.submenu && setExpandedItem(item.title)}
                        onMouseLeave={() => item.submenu && setExpandedItem(null)}
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-sm">{item.title}</span>
                          {item.submenu && (
                            <Plus 
                              className={`w-3 h-3 transition-transform ${expandedItem === item.title ? 'rotate-45' : ''}`}
                              style={{ strokeWidth: 3 }} 
                            />
                          )}
                        </div>
                        {item.submenu && expandedItem === item.title && (
                          <ul className="mt-2 pt-2 space-y-1 border-t" style={{ borderColor: 'rgb(55, 68, 109, 0.1)' }}>
                            {item.submenu.map((subitem) => (
                              <li key={subitem.title}>
                                <a
                                  href={subitem.href}
                                  className="block py-1.5 px-3 text-xs rounded hover:bg-gray-50 transition-colors"
